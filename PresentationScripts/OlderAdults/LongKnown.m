function LongKnown(id,song)

% EXPERIMENT STUFF:
%expdir  = strcat(basedir,'experiment\');
datadir = 'data';
stimdir = 'VolumeNormalized';
stimlist={'HeyJude-short_norm_mono.mp3', 'TwastheNight_norm.mp3'};
...
try
    ...
    soundfile = [stimdir filesep char(stimlist(song))];
    ...
    % SAVE DATA:
    save([datadir filesep 'LN' num2str(song) '_' id '_' Date '.mat'],'onsets')
    ...
    while ~keyIsDown
        [keyIsDown, endtime, keyCode] = KbCheck; %WTF IS HAPPENING HERE?
    end
    sca;
catch
    sca;
