...
function aa_user_familiarity_extract_OA
%add SPM and aa
...
cd(locpth);
load('seriesnumbers_OA.mat');
%ANALYSIS RECIPE
aap=aarecipe('aap_tasklist_familiarity_extract_OA.xml');

count =1;
%for id=1:length(pID);
for id=2; %structurals are in song 2 only
    %participant list in each song
    pID = dir('/imaging3/familiarity/OlderAdults/Session1/Song2/P*');
    %Location of raw data
    aap.directory_conventions.rawdatadir= sprintf('/imaging3/familiarity/OlderAdults/Session1/Song%d',id);
    aap.directory_conventions.T1template = '/home/asternin/spm12/templates/T1.nii';
    aap.acq_details.root = sprintf('/imaging3/familiarity/OlderAdults/Session1/Song%d',id);
...
%     aap.tasksettings.aamod_firstlevel_scrubbingmodel_BS.TR = 1.25;
    ...
    for p = 1:length(pID); % Num of participants
        ...
        aap=aas_addsubject(aap,sprintf('%s',pID(p).name),seriesnumbers(p,1)); %TR:1.25 
       
        struct_num = [16,14,16,15,14,16,15,14,15,15,15,16,15,14];% <- these are hardcoded change to so they correspond to T1 series number 
