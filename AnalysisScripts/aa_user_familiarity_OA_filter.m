...
for id=1%:length(sid)
    aap=[];
    aap=aarecipe('aap_tasklist_familiarity_OA_filter.xml'); %reinitialize aap so that add_subjects overwrites for each song
    ...
    for p=1:length(subj);        
        ...
        rootdir = sprintf('/imaging3/familiarity/OlderAdults/Session1/Song%d',sid(id)); 
        filedir = dir(sprintf('%s/Familiarity_analysis/aamod_smooth_00001/%s/*swrf*',rootdir,pID(p).name)); %structurals in Session1 song2 only
        root = sprintf('%s/Familiarity_analysis/aamod_smooth_00001/%s/',rootdir,pID(p).name);
        structlist = strcat(root,{filedir.name});
        aap=aas_addinitialstream(aap,'epi',sprintf('%s', pID(subj(p)).name), structlist);
